...
try {
    ...
    if (!$checkStmt->fetch()) {
        $_SESSION['project_error'] = 'Project not found or you do not have permission to edit images.';
        header('Location: manage_projects.php');
        exit;
    }
    ...
    if ($checkImagesTable->rowCount() > 0) {
        ...
        $pdo->prepare('UPDATE project_images SET is_primary = 0 WHERE project_id = :project_id')->execute([':project_id' => $project_id]);
        
        // Set this image as primary
        $updateStmt = $pdo->prepare('UPDATE project_images SET is_primary = 1 WHERE image_id = :image_id AND project_id = :project_id');
        $updateStmt->execute([':image_id' => $image_id, ':project_id' => $project_id]);
        
        if ($updateStmt->rowCount() > 0) {
            $_SESSION['project_success'] = 'Primary image updated successfully!';
        } else {
    ...
    $_SESSION['project_error'] = 'Error setting primary image: ' . $e->getMessage();
    header('Location: edit_project.php?id=' . $project_id);
    exit;
}
?>
